...
from eval_runtime_wer import word_edit_distance  # noqa: E402
...
def paired_bootstrap(rows: list[tuple[int, int, int]], n_boot: int = 10000, seed: int = 42) -> dict:
    """IC 95% (percentil) da diferença de WER e da melhora relativa, por reamostragem
    de utterances com reposição. `rows` = saída de per_utterance_errors."""
    ...
    for _ in range(n_boot):
        ...
        diffs.append(wb - wc)
        rels.append(100.0 * (wb - wc) / wb if wb else 0.0)
    diffs.sort()
    rels.sort()
...
def main() -> None:
    ...
    rows = per_utterance_errors(Path(args.baseline).read_text(), Path(args.candidate).read_text())
