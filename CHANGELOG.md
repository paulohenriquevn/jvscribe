...
- Baseline sobre test set 8 kHz (`scripts/run_baseline.py` + `scripts/baseline_minds14.py`): orquestra o test set + WER, rejeita pseudo-label (invariante `PRD.md` § 7.3), emite relatório com rótulo `[MEDIDO]`. Medição real sobre minds14 pt-PT (fala telefônica bancária real 8 kHz nativa, transcrição humana): **WER = 68,3% [IC95: 46,8%–96,9%]** para faster-whisper-base, com caveats honestos (pt-PT vs pt-BR, code-switching, modelo fraco = piso não teto) — `knowledge-base/measurements/m1-baseline-report.md`
