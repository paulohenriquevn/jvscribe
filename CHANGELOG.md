...
- `jvscribe/results/m6-realtime-current-model.md` **reetiquetado**: os RTFx de 41–90× ali
...
- Produto renomeado de "Macaw Voice" para **jvscribe** nos documentos vivos. Registros
  históricos (ADRs anteriores, `jvscribe/results/*.md`, medições) **não** foram reescritos —
...
### Known issues
- Os RTFx de 41–90× registrados em `jvscribe/results/m6-realtime-current-model.md` vêm de
...
- `MACAW_MODEL_DIR` também no runtime Rust (M9): `app.rs::model_dir()` passa a honrar a variável, fechando o item do DoD "nos dois lados" — antes só o lado Python o cumpria.
...
- Benchmark público reprodutível (`jvscribe/eval_public_hf.py`, `jvscribe/results/public-benchmarks.md`): **16,14% WER** no FLEURS pt_br (fala lida banda-larga), RTFx 24,6x em batch (CPU). Confirma o modelo forte em áudio de boa qualidade; o gargalo telefônico é canal/dado. [MEDIDO]
...
- Deliverable medido de M5 `[MEDIDO]` — Zipformer-CTC medium+fonema fine-tunado (CORAA+TAGARELA),
  ...
  Resultados + metodologia em `jvscribe/results/m5-final-results.md`; harness de medição em
...
- **M6 EXP-01 — int8 é lossless vs fp32 (custo zero de acurácia) `[MEDIDO]`** (lição T-Mimi testada): small finalista, mesma engine Rust, n=100 — int8 (27MB) WER 28,21%/CER 10,99% vs fp32 (92MB) WER 28,45%/CER 10,93%. Δ 0,24pp/0,06pp dentro do ruído → **int8 não degrada acurácia**, deploy do int8 (3,4× menor) é seguro. Refuta quantização mista/QAT p/ o nosso caso (nada a recuperar). Override `MACAW_MODEL` no runtime p/ A/B de modelo (`crates/macaw-cli/src/transcribe.rs`). Evidência: `jvscribe/results/runtime-eval-findings.md`.
