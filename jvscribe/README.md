# `jvscribe/` — as três pipelines do jvscribe
...
Outras pastas: [`tests/`](tests/) (suíte pytest — rode `python3 -m pytest jvscribe/tests` de
...
- **Nada de script solto na raiz.** Um teste-guarda (`tests/test_pipeline_layout.py`) falha se algum
  `.py`/`.sh` aparecer solto na raiz de `jvscribe/` ou se um módulo importar por nome outro de pipeline
  diferente (quebraria standalone).
- **Deps de canal telefônico** (`telephone_channel`, `codec_pool`) vivem em `jvscribe/corpus/` (nível
