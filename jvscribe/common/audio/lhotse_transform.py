...
determinístico dado um input fixo):
  ...
  3. Onde offline FAZ sentido — e é exatamente o que M3/M4/M5 já fazem — é
     para os TEST SETS (`jvscribe/make_telephone_test.py`,
     `jvscribe/make_coraa_test.py` da Task 3.1): lá não há augmentação por
...
convenção de `jvscribe/make_telephone_test.py`: o runbook de treino copia
