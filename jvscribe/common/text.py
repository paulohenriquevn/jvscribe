...
(`jvscribe/tests/test_text_normalization.py`): são **2 semânticas distintas**, com 3 cópias
...
def normalize_train_target(text: str) -> str:
    ...
    Fonte da verdade histórica: `jvscribe/finetune/prep_icefall.py:49`, replicada
    byte-a-byte em `jvscribe/tools/eval_runtime_wer.py:37` e
    `jvscribe/tools/analyze_error_composition.py:37`.
    """
...
def normalize_for_wer_compare(text: str) -> str:
    ...
    Delega à implementação canônica em `jvscribe/common/text_normalize_ptbr.py`, que tem os passos
