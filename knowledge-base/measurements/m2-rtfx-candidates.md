...
**Metodologia:** régua de M1 — **10 execuções + 3 de warmup descartadas**, reportando **média ± desvio, min–max e mediana**. A medição inicial só reportou mediana; `asr-evidence-discipline.md § 1` exige média ± desvio para `[MEDIDO]` — corrigido aqui (review F1). RTFx = duração_áudio ÷ tempo_de_parede (maior = mais rápido).
